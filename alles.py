...
with tab4:
    ...
    # Een lege lijst om de voorspellingen op te slaan per brandstofcategorie
    voorspellingen_per_categorie = []

    # Loop door de unieke brandstofcategorieën en bereken de voorspellingen voor elk
    for categorie in brandstofcategorieën:
        subset_data = cumulatief[cumulatief['Brandstof omschrijving'] == categorie]
        jaartal = subset_data['Bouwjaar'].values.reshape(-1, 1)
        aantal_voertuigen = subset_data["Aantal auto's"].values
        model = LinearRegression()
        model.fit(jaartal, aantal_voertuigen)
        voorspelling = model.predict(np.array([[2023]]))
        voorspellingen_per_categorie.append(voorspelling[0])

    # Maak een nieuwe DataFrame met de voorspellingen per brandstofcategorie voor 2023
    voorspellingen_df = pd.DataFrame({
        'Brandstof omschrijving': brandstofcategorieën,
        'Bouwjaar': [2023] * len(brandstofcategorieën),
        "Aantal auto's": voorspellingen_per_categorie
    })

    # Voeg de nieuwe DataFrame met voorspellingen toe aan je oorspronkelijke DataFrame
    cumulatief = pd.concat([cumulatief, voorspellingen_df], ignore_index=True)

    # Druk de voorspellingen af
    print("Voorspellingen per brandstofcategorie voor 2023:")
    print(cumulatief.tail(len(brandstofcategorieën)))  # Toon alleen de voorspellingen voor 2023 per brandstofcategorie
...
with tab4:
    ...
    diesel = cumulatief[(cumulatief['Brandstof omschrijving'] == 'Diesel') & (cumulatief['Bouwjaar'] > 2018)]
    benzine = cumulatief[(cumulatief['Brandstof omschrijving'] == 'Benzine') & (cumulatief['Bouwjaar'] > 2018)]
    hybride = cumulatief[(cumulatief['Brandstof omschrijving'] == 'Hybride') & (cumulatief['Bouwjaar'] > 2018)]
    elektriciteit = cumulatief[(cumulatief['Brandstof omschrijving'] == 'Elektriciteit') & (cumulatief['Bouwjaar'] > 2018)]
